...
- **Payments Section (full page)**: Sidebar → Payments with 4 tabs: Dashboard (KPI cards, revenue trend chart, provider breakdown pie, recent transactions, status breakdown), Transactions (filterable/searchable table with provider/status/date filters, pagination), Analytics (daily revenue bar chart, success rate horizontal bar chart, revenue by provider progress bars, summary metrics), and Payment Links (create/manage shareable links with generate dialog, copy/cancel/delete actions). Tables: `crm_payment_transactions` (120 seeded demo rows), `crm_payment_links` (15 seeded demo links). Backend routes: `GET /api/payments/stats`, `GET /api/payments/transactions`, `GET /api/payments/links`, `POST /api/payments/links`, `PATCH /api/payments/links/:id`, `DELETE /api/payments/links/:id`.
